...
public class SetFlagViaRest {
	...
	public final static void main(String[] args) throws Exception {
		...
		// Delete
		try {
			method = buildSetFlagMethod(hostname, port, user, password, cid, did, fid);
     ...
     private static HttpMethod buildSetFlagMethod(final String hostname, final int port, final String user, final String password, final String cname, String did, String fid) throws FileNotFoundException {
         ...
         parts.add(new StringPart("id", fid)); // not the string name like "testF" 
         parts.add(new StringPart("collection", cname));
         parts.add(new StringPart("output", "application/json"));
         parts.add(new StringPart("uri", did));

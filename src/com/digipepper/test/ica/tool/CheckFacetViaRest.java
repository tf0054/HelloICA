...
public class CheckFacetViaRest {
    ...
    public final static void main(String[] args) throws Exception {
		...
		CheckFacetViaRest shell = new CheckFacetViaRest();
        CmdLineParser parser = new CmdLineParser(shell);
        try {
            parser.parseArgument(args);
        } catch (CmdLineException e) {
            System.out.println("Usage");
            System.out.println(" CheckViaRest -hostname <hostname> -port <port> -username <username> -password <password> -collectionId <collectionId>");
            System.out.println();
     ...
     private static HttpMethod buildHttpMethod(final String hostname, final int port, final String user, final String password, final String cname) throws FileNotFoundException {
        final StringBuffer url = new StringBuffer("http://");
        //url.append(hostname).append(":").append(port).append("/api/v10/admin/collection?method=getConfig");
        //url.append(hostname).append(":").append(port).append("/api/v10/admin/pear?method=getList");
        //url.append(hostname).append(":").append(port).append("/api/v10/admin/field?method=getList");
        url.append(hostname).append(":").append(port).append("/api/v10/admin/facetTree?method=getFacetTree");
        //url.append(hostname).append(":").append(port).append("/api/v10/admin/document?method=getDocumentStatus");
        //url.append(hostname).append(":").append(port).append("/api/v10/analysis/typesystem");
        ...
        parts.add(new StringPart("status", "200"));
        parts.add(new StringPart("numResults", "1000"));
